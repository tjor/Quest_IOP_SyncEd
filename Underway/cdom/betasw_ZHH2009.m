...
% used.
# theta: angles in degress (to speed up computations)
# cos_theta 2= cos(deg2rad(theta)).^2 (to speed up computations)
...
#     betasw(:,i) = beta90sw(i)*(1+((cos(rad)).^2).*(1-delta)/(1+delta)); 
    betasw(:,i) = beta90sw(i)*(1+(cos_rad2).*(1-delta)/(1+delta)); # this is to try to speed up execution time
end
